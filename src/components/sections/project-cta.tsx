...
export async function ProjectCta() {
  ...
  const headshotUrl = urlFor(settings.headshot)?.width(160).height(160).fit("crop").url();
    ...
    <section className="bg-ink">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-10 px-5 py-16 sm:px-11 sm:py-24 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
        <Reveal>
          ...
          <div className="mt-[30px] flex flex-wrap items-center gap-[14px]">
            <BookingTrigger
              ...
              className="bg-paper text-ink flex items-center gap-[9px] rounded-full px-[26px] py-[15px] text-sm font-medium transition-opacity hover:opacity-90"
            >
              Book a 20-min call
            ...
            <a href={`mailto:${settings.email}`} className="text-paper-70 font-mono text-[13px]">
              {settings.email}
            </a>
          </div>
        </Reveal>
        ...
        <Reveal className="flex flex-none items-center gap-4">
          ...
          <div>
            <p className="text-paper text-sm font-medium">{settings.name}</p>
